import { Compass, Loader2, Radio, Server, X } from 'lucide-react'
...
export const ProfileNodesModal: React.FC<ProfileNodesModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-150">
      <div className="bg-card border border-border rounded-xl shadow-xl w-full max-w-2xl max-h-[85vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-150">
        {/* Header */}
        ...
        <div className="px-5 py-3 border-b border-border bg-card/50">
          <input
            ...
            className="w-full px-3 py-1.5 rounded-md bg-background border border-border text-xs text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-primary"
          />
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
              {filteredNodes.map((node) => (
                <div
                  ...
                  className="p-3 rounded-lg border border-border bg-background/50 hover:bg-background/80 transition-colors flex items-center justify-between gap-3 text-xs"
                >
        ...
        <div className="px-5 py-3 border-t border-border flex items-center justify-between text-xs text-muted-foreground bg-card/50">
          ...
          <button
            type="button"
            onClick={onClose}
            className="px-3.5 py-1.5 rounded-md border border-border bg-background text-foreground hover:bg-accent transition-colors"
          >
            关闭
          </button>
        </div>
